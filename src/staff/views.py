    ...
    def post(self, request, *args, **kwargs):
        ...
        if order_id:
            ...
            if order:
                ...
                return redirect('staff:order-detail', order_id)
            else:
...
class OrderDetailInvoiceView(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        order = get_object_or_404(Order.objects.filter(status='pai'), transaction_id=kwargs['tx_id'])
        return super(OrderDetailInvoiceView, self).dispatch(request)
